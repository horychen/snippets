...
s = 3*0 + 1 *np.sin(frequency*1  *2*np.pi*t + 120/180*np.pi) \
      + 0*2 *np.sin(frequency*5  *2*np.pi*t) \
      + 0*3 *np.sin(frequency*7  *2*np.pi*t) \
      + 0*4 *np.sin(frequency*7.5*2*np.pi*t) \
      + 0*4 *np.sin(frequency*8  *2*np.pi*t) \
      + 0*4 *np.sin(frequency*8.5*2*np.pi*t) \
      + 0*5 *np.sin(frequency*50 *2*np.pi*t) # 混叠示意（需要先抗混叠滤波）

fig, axes = plt.subplots(3, dpi=150)
axes[0].plot(t, s, '-o')
...
if True:
    ...
    spectrum = np.abs(sfft)*2/ns 
    specturm_phase = np.angle(sfft) / np.pi*180
    specturm_phase_cleanup = [phase*(ampl>1e-3) for ampl, phase in zip(spectrum, specturm_phase)]

    axes[1].plot(w, spectrum, '.')
    for index,(a,b) in enumerate(zip(w, spectrum)):
        print(f'{index} | {a} Hz | ' + f'{b:g} V' if b>1e-14 else f'{index} | {a} Hz | ')

    axes[2].plot(w, specturm_phase_cleanup, '.')
